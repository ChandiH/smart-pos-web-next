...
const ProductVariantTable = ({
  ...
  const [deletedIds, setDeletedIds] = useState<Product_Variant["variant_id"][]>([]);
  const [sorting, setSorting] = useState<SortingState>([]);
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);
  const [columnVisibility, setColumnVisibility] = useState<VisibilityState>({});
  const [rowSelection, setRowSelection] = useState({});

  const refreshData = () => setData(() => [...variants]);

  const handleSave = async () => {
    if (data.length > 0) {
      try {
        const updatedVariants = data.filter((variant) => !variant.variant_id.startsWith("new-"));
        const promise = updateProductVariants(updatedVariants);
        Toast.promise(promise, {
          loading: "Updating…",
          success: "Variants updated",
          error: (error) => error?.response?.data?.error ?? "Failed to update variant",
        });
        await promise;
      } catch (error) {
        Toast.error("Error Update Product Variants:");
        console.error("Error: Update Variants", error);
      }
    }

    if (data.length > 0) {
      try {
        const newVariants: ProductVariantAddRequest[] = data
          .filter((variant) => variant.variant_id.startsWith("new-"))
          .map((variant) => ({
        ...
        const promise = addProductVariants(newVariants);
        Toast.promise(promise, {
          loading: "Updating…",
          success: "Variants updated",
          error: (error) => error?.response?.data?.error ?? "Failed to update variant",
    ...
    if (deletedIds.length > 0) {
      try {
        const promise = deleteProductVariants(deletedIds);
        Toast.promise(promise, {
          loading: "Deleting…",
          success: "Variants deleted",
          error: (error) => error?.response?.data?.error ?? "Failed to delete variant",
        });
        await promise;
      } catch (error) {
        Toast.error("Error Delete Product Variants:");
        console.error("Error: Delete Variants", error);
      }
    }
  };
  ...
  const handleVariantDelete = (variant_id: Product_Variant["product_id"]) => {
    ...
    setDeletedIds((prev) => [...prev, variant_id]);
  };
  ...
  const columns: ColumnDef<Product_Variant>[] = [
    ...
    },
    {
      ...
      header: "Delete",
      cell: ({ row }) => (
        <Button variant="destructive" onClick={() => handleVariantDelete(row.getValue("variant_id"))}>
          <Trash2 />
        </Button>
      ),
    },
    ...
    <div className="w-full space-y-4">
      ...
      <div className="text-muted-foreground flex items-center justify-between gap-2 text-sm max-md:flex-col">
        <div className="flex flex-row items-center gap-4">
          <Button variant="outline" size="sm" onClick={handleAddNewVariant}>
            Add New Variant
          </Button>
